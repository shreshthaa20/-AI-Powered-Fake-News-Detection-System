...
A comprehensive system for detecting misinformation across multiple formats (text, images, videos) and multiple languages (including Indian languages).

## ✨ Features

| Feature | Description |
|---------|-------------|
| **Text Analysis** | BERT-based classification + pattern detection (clickbait, emotional manipulation, unverified claims, source credibility) |
| **Image Analysis** | EXIF metadata, manipulation heuristics, color inconsistencies, perceptual hashing |
| **Video Analysis** | OpenCV frame extraction, face detection, deepfake heuristics (unnatural smoothing), quality inconsistency detection |
| **Fact Checking** | Google Fact Check Tools API integration + viral misinfo pattern detection |
| **Multilingual Support** | 11 Indian languages + English with live translation |
| **Credibility Scoring** | 0-1 score with classification (Highly Credible → Likely False) |
| **Evidence Generation** | Provides explanations for all decisions |
...
│   ├── text_analyzer.py        # Text + BERT fake-news classifier
│   ├── image_analyzer.py       # Image manipulation detection
│   ├── video_analyzer.py       # Video frame analysis + deepfake heuristics
│   ├── credibility_scorer.py  # Credibility scoring engine
│   ├── multilingual_processor.py  # Multi-language support + translation
│   ├── fact_checker.py         # Google Fact Check API + heuristics
